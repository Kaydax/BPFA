...
bool MIDIOutDevice::Open(int iDev)
{
	...
	/*HMODULE ks = LoadLibraryA("OmniMIDI/OmniMIDI");
	...
	{
		FreeLibrary(ks);
		puts("Please Enable KDMAPI From The OmniMIDI Configurator");
		return 1;
	}
...
{
	MMRESULT(WINAPI * KShortMsg)(DWORD) = (MMRESULT(WINAPI*)(DWORD))GetProcAddress(0, "SendDirectData");
	if (!m_bIsOpen) return false;

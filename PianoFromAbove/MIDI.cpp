...
typedef BOOL(WINAPI* IKDMAPIS)();
typedef BOOL(WINAPI* TKDMAPIS)();
typedef VOID(WINAPI* RKDMAPIS)();
typedef UINT(WINAPI* SDD)(DWORD);

static HMODULE OM = 0;
static IKDMAPIS InitializeKDMAPIStream = 0;
static TKDMAPIS TerminateKDMAPIStream = 0;
static RKDMAPIS ResetKDMAPIStream = 0;
static SDD SendDirectData = 0;
...
bool MIDIOutDevice::Open(int iDev)
{
  ...
  OM = LoadLibraryW(L"OmniMIDI\\OmniMIDI");
  if(OM)
  {
    InitializeKDMAPIStream = (IKDMAPIS)GetProcAddress(OM, "InitializeKDMAPIStream");
    TerminateKDMAPIStream = (TKDMAPIS)GetProcAddress(OM, "TerminateKDMAPIStream");
    ResetKDMAPIStream = (RKDMAPIS)GetProcAddress(OM, "ResetKDMAPIStream");
    SendDirectData = (SDD)GetProcAddress(OM, "SendDirectData");
  }

  if(InitializeKDMAPIStream())
  {
    printf("OmniMIDI initialize!");
    return true;
  }
  else
  {

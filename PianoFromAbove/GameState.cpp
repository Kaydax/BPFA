...
void SplashScreen::InitState()
{
	...
	SetChannelSettings(vector< bool >(), vector< bool >(), vector< bool >(),
		vector< unsigned >(cVisual.colors, cVisual.colors + sizeof(cVisual.colors) / sizeof(cVisual.colors[5])));
	...
	if (OptimizedModeEnabled == false)
	{
		...
		else
		{
			...
			if (!m_vState.empty()) {
				...
				m_vState[state_map[pEvent->GetSister()]] = m_iStartPos;
			}
			//}
...
void MainScreen::RenderStatus(LPRECT prcStatus)
{
	...
	// Build the ver text
	TCHAR sBPFAVer[256];
	_stprintf_s(sBPFAVer, TEXT("BPFA V2 Pre Release 2"), isOptimizedModeEnabled);

	// Build the Scoring text
	TCHAR sScore[256] = TEXT("N/A"), sMult[256] = TEXT("");
	...
	TCHAR sLearn[256] = TEXT("All Tracks"), * sMode = (m_eLearnMode == GameState::Adaptive ? TEXT("Adaptive") : TEXT("Waiting"));

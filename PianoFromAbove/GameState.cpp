...
const wchar_t* build_version = L"BPFA V2 Pre Release 3 | Dev Build 3";
...
bool OptimizedModeEnabled = false;
bool RM3D = false;
bool toggleText = false;
string isOptimizedModeEnabled;
...
void SplashScreen::InitState()
{
  ...
  SetChannelSettings(vector< bool >(), vector< bool >(), vector< unsigned >(cVisual.colors, cVisual.colors + sizeof(cVisual.colors) / sizeof(cVisual.colors[0])));
...
void SplashScreen::RenderNote(int iPos)
{
  ...
  // Visualize!
  long long llDuration = m_llStartTime - (m_MIDI.GetInfo().llFirstNote - 0);
...
MainScreen::MainScreen(wstring sMIDIFile, State eGameMode, HWND hWnd, Renderer* pRenderer) : GameState(hWnd, pRenderer), m_MIDI(sMIDIFile), m_eGameMode(eGameMode)
{
...
void MainScreen::InitColors()
{
  ...
  m_csKBRed.SetColor(0xF00000, 0.5f);
  ...
  switch(msg)
  {
    ...
    case WM_KEYDOWN:
    {
      ...
      switch(wParam)
      {
        ...
        case 'G': //Optimized Mode
          for(int i = 0; i < 256; i++)
            m_pNoteState[i] = -1;
          m_vState.clear();
          OptimizedModeEnabled = !OptimizedModeEnabled;
          return Success;
        case 'H': //Toggle BPFA text
          toggleText = !toggleText;
          return Success;
        case 'X': //Reset Notes Played counter
          NotesPlayed = 0;
          return Success;
        case 'Y': //IDFK, break the program?
          JumpTo(static_cast<long long>(m_llStartTime - cControls.dFwdBackSecs * 100000000000));
          RenderingMode = !RenderingMode;
          return Success;
        case 'B': //Toggle note outlines
          OutlinesEnabled = !OutlinesEnabled;
          return Success;
        case 'J': //Enable "amazing" 3D mode
          RM3D = !RM3D;
          return Success;
      }
      break;
...
GameState::GameError MainScreen::Logic(void)
{
  /*if (NPS > 10000)
  {
    ...
    for (int i = 0; i < 256; i++)
      m_pNoteState[i] = -1;
    JumpTo(static_cast<long long>(m_llStartTime + 3.0000000000000000 * 1000000));
    JumpTo(static_cast<long long>(m_llStartTime - 3.0000000000000000 * 1000000));
  }*/
  static Config& config = Config::GetConfig();
  static PlaybackSettings& cPlayback = config.GetPlaybackSettings();
  ...
  if(m_llFPSTime >= 500000)
  {
    ...
    m_avgFPS = 0.9 * m_avgFPS + (1.0 - 0.9) * m_dFPS;
...
{
  if(OptimizedModeEnabled == true)
  {
    MIDIChannelEvent* pEvent = m_vEvents[iPos];
    MIDIChannelEvent::ChannelEventType eEventType = pEvent->GetChannelEventType();
    if(eEventType == MIDIChannelEvent::NoteOn)
    {
      NotesPlayed += 1;
      TNPS++;
    }
  }
  if(OptimizedModeEnabled == false)
  {
    // Event data
    MIDIChannelEvent* pEvent = m_vEvents[iPos];
    if(!pEvent->GetSister()) return;
...
MIDIMetaEvent* MainScreen::GetPrevious(eventvec_t::const_iterator& itCurrent, const eventvec_t& vEventMap, int iDataLen)
{
  const MIDI::MIDIInfo& mInfo = m_MIDI.GetInfo();
...
void MainScreen::RenderNote(thread_work_t& work)
{
  ...
  if(OutlinesEnabled == true)
  {
    reinterpret_cast<D3D9Renderer*>(m_pRenderer)->GenRect(x, y - cy, cx, cy, csTrack.iVeryDarkRGB, &batch_vertices[work.queue_pos]);
  }
  
  reinterpret_cast<D3D9Renderer*>(m_pRenderer)->GenRect(x + fDeflate, y - cy + fDeflate, cx - fDeflate * 2.0f, cy - fDeflate * 2.0f, csTrack.iPrimaryRGB, csTrack.iDarkRGB, csTrack.iDarkRGB, csTrack.iPrimaryRGB, &batch_vertices[work.queue_pos + 6]);
}
...
void MainScreen::RenderText()
{
  ...
  // Screen info
  RECT rcStatus = { m_pRenderer->GetBufferWidth() - 156, 1, m_pRenderer->GetBufferWidth(), 6 + 16 * iLines };
...
void MainScreen::RenderStatus(LPRECT prcStatus)
{
  ...
  // Build the Avg FPS text
  TCHAR sAvgFPS[256];
  _stprintf_s(sAvgFPS, TEXT("%.1lf"), m_avgFPS);

  // Build the NotesPlayed text
  TCHAR sNotesPlayed[256];
  _stprintf_s(sNotesPlayed, TEXT("%llu"), NotesPlayed);

  // Build the NPS text
  TCHAR sNPS[256];
  _stprintf_s(sNPS, TEXT("%llu"), NPS);
  ...
  // Build the OptimizedMode text
  TCHAR sOptimizedMode[256];
  if(OptimizedModeEnabled == true)
    _stprintf_s(sOptimizedMode, TEXT("\nOptimized Mode: Enabled"), isOptimizedModeEnabled);
  else
    _stprintf_s(sOptimizedMode, TEXT("\nOptimized Mode: Disabled"), isOptimizedModeEnabled);

  // Build the OutlinesEnabled text
  TCHAR sOutlinesEnabled[256];
  if(OutlinesEnabled == true)
    _stprintf_s(sOutlinesEnabled, TEXT("\n\nNote Outlines: Enabled"), isOptimizedModeEnabled);
  else
    _stprintf_s(sOutlinesEnabled, TEXT("\n\nNote Outlines: Disabled"), isOptimizedModeEnabled);

  // Build the ver text
  TCHAR sBPFAVer[256];
  _stprintf_s(sBPFAVer, build_version, isOptimizedModeEnabled);

  // Display the text
  InflateRect(prcStatus, -6, -6);
  ...
  WCHAR noteprintbuf[20];
  OffsetRect(prcStatus, 2, 16 + 3);
  m_pRenderer->DrawText(TEXT("Notes Played: "), Renderer::Small, prcStatus, 0, 0xFF404040);
  m_pRenderer->DrawText(sNotesPlayed, Renderer::Small, prcStatus, DT_RIGHT, 0xFF404040);
  OffsetRect(prcStatus, -2, -3);
  m_pRenderer->DrawText(TEXT("Notes Played: "), Renderer::Small, prcStatus, 0, 0xFFFFFFFF);
  m_pRenderer->DrawText(sNotesPlayed, Renderer::Small, prcStatus, DT_RIGHT, 0xFFFFFFFF);
  ...
  if(m_bShowFPS)
  {
    ...
    m_pRenderer->DrawText(TEXT("AVG FPS:"), Renderer::Small, prcStatus, 0, 0xFF404040);
    m_pRenderer->DrawText(sAvgFPS, Renderer::Small, prcStatus, DT_RIGHT, 0xFF404040);
    OffsetRect(prcStatus, -2, -2);
    m_pRenderer->DrawText(TEXT("AVG FPS:"), Renderer::Small, prcStatus, 0, 0xFFFFFFFF);
    m_pRenderer->DrawText(sAvgFPS, Renderer::Small, prcStatus, DT_RIGHT, 0xFFFFFFFF);
  ...
  if(toggleText == false)
  {
    ...
    OffsetRect(TestStatus, 0, 0);
    m_pRenderer->DrawText(sOptimizedMode, Renderer::Small, TestStatus, 0, 0xFFFFFFFF);

    OffsetRect(TestStatus, 0, 0);
    m_pRenderer->DrawText(sOutlinesEnabled, Renderer::Small, TestStatus, 0, 0xFFFFFFFF);

    OffsetRect(TestStatus, 0, 0);
    m_pRenderer->DrawText(sBPFAVer, Renderer::Small, TestStatus, 0, 0xFFFFFFFF);
  }
}

...
{
  int pDialogs[] = { IDD_PP1_VISUAL, IDD_PP2_AUDIO, IDD_PP4_CONTROLS, IDD_PP3_VIDEO};
  DLGPROC pProcs[] = { VisualProc, AudioProc, ControlsProc, VideoProc};
  LPCWSTR pTitles[] = { TEXT("Visual"), TEXT("Audio"), TEXT("Controls"), TEXT("Video") };
  PROPSHEETPAGE psp[4];
  PROPSHEETHEADER psh;
...
INT_PTR WINAPI VideoProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
  ...
  case WM_INITDIALOG:
  {
    ...
    CheckRadioButton(hWnd, IDC_DIRECT3D, IDC_VULKAN, IDC_DIRECT3D + cVideo.eRenderer);
  ...
  case WM_NOTIFY:
  {
    ...
    case PSN_APPLY:
    {
      ...
      cVideo.eRenderer = (IsDlgButtonChecked(hWnd, IDC_DIRECT3D) == BST_CHECKED ? cVideo.Direct3D :
        ...
        IsDlgButtonChecked(hWnd, IDC_VULKAN) == BST_CHECKED ? cVideo.VULKAN :
        cVideo.Direct3D);
